' ============================================================================
' Main_ISLLSYS_20010101.vbs — clean stitched reconstruction
' Source: VS session (solution AQS.Advantage / project AQS.Advantage.BatchEdit),
' photos IMG_4434-IMG_4660 (one continuous top-to-bottom scroll, no gutter line numbers).
' Stitched band-by-band with photo-seam overlaps de-duplicated. Band seams are marked.
' Raw photo-delimited ground-truth: main-isllsys-RAW-concatenated.vbs.md
' ============================================================================

' ---- band seam: IMG_4434..IMG_4447 (first: window_onload, last: LogoutMain) ----
...
'<<< GAP: possible unphotographed lines between IMG_4445 and IMG_4446 >>>
        Set objFrames = Nothing
...
Sub AddWindowToCollection(myWindow)
    ...
    End If
End Sub
'<<< GAP: possible unphotographed lines between IMG_4446 and IMG_4447 >>>
Function LogoutMain()
'****************************************************************************
...
' ---- band seam: IMG_4448..IMG_4461 (first: continuation, last: DeleteAction) ----
                Set objWindowLogoutMain = arrWindowsMain(intCount)
                If Not objWindowLogoutMain Is Nothing Then
                    If Not objWindowLogoutMain.closed Then
                        objWindowLogoutMain.close
...
'<<< GAP: possible unphotographed lines between IMG_4452 and IMG_4453 >>>
...
'<<< GAP: possible unphotographed lines between IMG_4455 and IMG_4456 >>>
...
'<<< GAP: possible unphotographed lines between IMG_4456 and IMG_4457 >>>
        Else
            objFrames(lngCounter).tabIndex = -1
        ...
        If Not TrapXMLParseError(objResponse, "DeleteAction|3") Then
            ...
            If left(UCase(GetNodeText(objNode, "status", "")),5) = "ERROR" Then
                ...
                If left(UCase(strMessage),5) = "BATCH" Then
                    ...
                    With mxmlBatchEdits
'<<< GAP: possible unphotographed lines between IMG_4460 and IMG_4461 >>>
                End With
                Call ShowBatchEdits("")
...
' ---- band seam: IMG_4462..IMG_4475 (first: continuation, last: ExecuteAction) ----
                        strNLOCNUM = GetStringField(pstrNodeKey, 7, "|")
...
'<<< GAP: possible unphotographed lines between IMG_4472 and IMG_4473 >>>
...
' ---- band seam: IMG_4476..IMG_4489 (first: continuation, last: UserOption) ----
strWindowName = GetNodeText(objNode, "name", "policy")
...
If StrComp( GetNodeText(objNode, "UseExistingQueryString", ""), "no", vbTextCompare) = 0 Then
    ...
    If StrComp( strTempQueryString, "", vbTextCompare) <> 0 Then
        ...
        strQueryString = strQueryString & "&" & strTempQueryString
    End If
'<<< GAP: possible unphotographed lines between IMG_4476 and IMG_4477 >>>
...
' ---- band seam: IMG_4490..IMG_4503 (first: continuation, last: RefreshSearchPageLists) ----
    Dim strXSL
    Dim nodItem
    Dim xmlUserOptionsXML
...
'*******************************************************************************
    ...
    Set objDoc = document.frames("tree")
'<<< GAP: possible unphotographed lines between IMG_4494 and IMG_4495 >>>
        On Error Resume Next
        Set objDoc = document.frames("tree")
        If Not objDoc Is Nothing Then
...
' ---- band seam: IMG_4504..IMG_4517 (first: continuation, last: ApplySecurityPolicy) ----
        End If
...
' ---- band seam: IMG_4518..IMG_4531 (first: continuation, last: ExecuteBrowserCommands) ----
Next
...
Dim blnCheckRating                         'Used to identify controls that affect rating
    ...
    If blnCallServer Then
        ...
        .marrEEData(1) = objControl.name
            'Set the matchode
        .marrEEData(5) = "1"
'<<< GAP: possible unphotographed lines between IMG_4520 and IMG_4521 >>>
    End With
        ' objCallingWindow
...
'<<< GAP: possible unphotographed lines between IMG_4521 and IMG_4522 >>>
...
' ---- band seam: IMG_4532..IMG_4545 (first: continuation, last: continuation) ----
                    objOption.setwidth()
                        Call TrapBrowserError("EEBrowser|ExecuteBrowserCommands|ADD_LISTITEM|2")
                    End If
...
' ---- band seam: IMG_4546..IMG_4559 (first: continuation, last: FillCallArray) ----
                strModalSize = .mstrModalSize
    ...
    Case "SET_VISIBILITY", "SET_VISIBILITY_PARENT"
        ...
        '// Added for functional security
'<<< GAP: possible unphotographed lines between IMG_4553 and IMG_4554 >>>
Call GetSecurityValues(strProgramType,strPageMatchCode,strNoun,blnSecDisabled,blnSecVisible)
End With
'// End add for functional security
...
' ---- band seam: IMG_4560..IMG_4573 (first: FillComboList, last: GetControlValue) ----
    End Sub
'------------------------------------------------------------------------------
...
Sub FillSelectList(ByRef objControl, ByRef objNodes, ByVal strRelatedList, ByRef objCallingWindow)
    ...
    End If
End Sub
'<<< GAP: possible unphotographed lines between IMG_4564 and IMG_4565 >>>
    '------------------------------------------------------------------
...
' ---- band seam: IMG_4574..IMG_4587 (first: continuation, last: GetXMLData) ----
                'UCase(objControl.type)
...
'<<< GAP: possible unphotographed lines between IMG_4577 and IMG_4578 >>>
        Dim strItemXmlList
        Dim strItemAction
        Dim strItemText
        Dim strItemEvent
...
'<<< GAP: possible unphotographed lines between IMG_4582 and IMG_4583 >>>
        End If
...
' ---- band seam: IMG_4588..IMG_4601 (first: continuation, last: NavButtonOnClick) ----
        End If
        Call TrapBrowserError("EEBrowser|GetXMLData|8")
...
' ---- band seam: IMG_4602..IMG_4615 (first: continuation, last: OnKeyPressHandler) ----
'Set focus to the cancel button if it is visible and not disabled.
If Not .dtaCancel.disabled and .dtaCancel.style.visibility = "visible" Then
    .dtaCancel.focus
End If
...
' ---- band seam: IMG_4616..IMG_4629 (first: continuation, last: SetInitialValueData) ----
' If both conditions are true Or both conditions are false, no need to check.
    Call CheckRequiredIndicators("",objCallingWindow)
End If
...
' ---- band seam: IMG_4630..IMG_4643 (de-dup: SetInitialValueData declaration was split across this seam; partial leading copy removed) ----
...
' ---- band seam: IMG_4644..IMG_4657 (first: continuation, last: ShowZeroText) ----
...
Sub SetControlValue(ByRef objControl, ByVal strMatchCode, ByVal vntValue, ByVal strVarType, ByRef objCallingWindow)
    ...
    If Not objControl Is Nothing Then
        ...
        Select Case UCase(objControl.tagname)
            ...
            Case "COMBO", "KPCOMBO"
                Select Case strVarType
'<<< GAP: possible unphotographed lines between IMG_4650 and IMG_4651 >>>
...
'<<< GAP: possible unphotographed lines between IMG_4656 and IMG_4657 >>>
...
' ---- band seam: IMG_4658..IMG_4660 (first: ShowZeroText, last: ClearXMLDetail) ----
    Function ShowZeroText(ByVal pobjControl)
    '****************************************************************************
    ' This function provides the ability to control the "NONE" text when
    ' a combo needs to shown NONE.
